...
## ISSUE-008: 팀 정보 저장
- 매번 팀 정보를 다시 입력해야 하는 것이 불편합니다.
- 이전에 입력했던 적이 있다면, 팀 이름과 라인별 선수들 이름을 저장해서, 다음 접속 때도 사용할 수 있어야 합니다.
- 저장된 팀 목록이 팀 설정 하단에 표시되어 있고, 이를 드래그해서 블루 또는 레드 팀 창에 놓으면, 해당 정보가 입력되도록 합니다.
- 이름들을 변경할 수 있도록 저장 버튼을 팀별로 만듭니다. 해당 버튼을 누르면 이름들이 해당 팀으로 저장됩니다.
- 저장된 팀 정보는 나중에 다시 접속했을 때 사용할 수 있어야 합니다.
